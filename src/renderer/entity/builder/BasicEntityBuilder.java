...
public class BasicEntityBuilder {
    ...
    public static IEntity createDiamond(Color color, double size, double centerX, double centerY, double centerZ){
        ...
        int edges = 20;
        double inFactor = 0.8; //inner/outer ring ratio
        ...
        for(int i = 0; i < edges; i++){
            ...
            double zPos = size/2;
            outerPoints[i] = new MyPoint(centerX + xPos, centerX + yPos, centerZ + zPos * inFactor);
            innerPoints[i] = new MyPoint(centerX + xPos * inFactor, centerX + yPos * inFactor, centerZ + zPos);
        }
